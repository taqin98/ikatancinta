...
export const PUBLIC_TEMPLATE_PREFIX = `${normalizedBase}templates/exclusive/puspa-asmara/`;
...
export function resolveAssetUrl(path) {
  ...
  if (uploadsMatch) {
    ...
    if (/\.json$/i.test(filename)) return `${PUBLIC_ASSET_PREFIX}lottie/${filename}`;
...
export function slideToggleElement(element, duration = 1000) {
  ...
  const isOpen = element.classList.contains("pa-open");
  if (isOpen) {
    element.style.maxHeight = `${element.scrollHeight}px`;
    element.style.opacity = "1";
    requestAnimationFrame(() => {
      element.classList.remove("pa-open");
      element.style.maxHeight = "0px";
      element.style.opacity = "0";
    });
    window.setTimeout(() => {
      if (!element.classList.contains("pa-open")) {
...
export function formatAddressHtml(addressName) {
  const line1 = normalizeText(addressName);
  return [line1].filter(Boolean).map((item) => `<p><b>${escapeHtml(item)}</b></p>`).join("");
}
