...
const PUBLIC_ASSET_PREFIX = `${normalizedBaseUrl}templates/basic/timeless-promise/assets/`;
...
export default function TimelessPromiseTemplate({
    ...
    useEffect(() => {
        ...
        const backgroundFallbacks = [
            [".elementor-element-1151be1e", "assets/image/ornament/5-FILEminimizer-e1703737895919-1.jpg"],
            [".elementor-element-36d72a88", "assets/image/ornament/5-FILEminimizer-e1703737895919-1.jpg"],
            [".elementor-element-2a734d13", "assets/image/couple/fres-WANITA-1.jpg"],
            [".elementor-element-1be20715", "assets/image/couple/fres-PRIA-1.jpg"],
        ];
        backgroundFallbacks.forEach(([selector, assetPath]) => {
            const node = root.querySelector(selector);
            if (!node) return;
            if (window.getComputedStyle(node).backgroundImage !== "none") return;
            node.style.backgroundImage = `url("${resolveAssetUrl(assetPath)}")`;
