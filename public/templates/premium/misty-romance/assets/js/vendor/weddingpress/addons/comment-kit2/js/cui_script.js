jQuery(document).ready(function ($) {
  ...
  var CUI_DUMMY_COUNT = 5;
  var CUI_DUMMY_AVATAR = 'assets/images/branding/logo-32x32.jpg';

  function escapeHtml_CUI(value) {
    return String(value || '')
      .replace(/&/g, '&amp;')
      .replace(/</g, '&lt;')
      .replace(/>/g, '&gt;')
  ...
  function buildDummyCommentList_CUI(submittedData) {
    var defaults = [
      { author: 'Ayu', comment: 'Selamat menempuh hidup baru. Samawa selalu ya.', konfirmasi: 'Hadir', time: 'Baru saja' },
      { author: 'Rizky', comment: 'Doa terbaik untuk kedua mempelai. Bahagia selalu.', konfirmasi: 'Hadir', time: '2 menit lalu' },
      { author: 'Nina', comment: 'Masya Allah, semoga dilancarkan sampai hari H.', konfirmasi: 'Tidak hadir', time: '5 menit lalu' },
      { author: 'Dimas', comment: 'Semoga jadi keluarga yang sakinah, mawaddah, warahmah.', konfirmasi: 'Hadir', time: '8 menit lalu' },
      { author: 'Tamu Undangan', comment: 'Turut berbahagia. Semoga selalu diberkahi.', konfirmasi: 'Tidak hadir', time: '12 menit lalu' }
    ];
    if (submittedData && submittedData.comment) {
      defaults[0] = {
        author: submittedData.author || 'Tamu Undangan',
        comment: submittedData.comment,
        konfirmasi: submittedData.konfirmasi || 'Hadir',
        time: 'Baru saja'
      };
    }
    return defaults.slice(0, CUI_DUMMY_COUNT);
  }

  function buildDummyCommentItem_CUI(comment, post_id, index) {
    var id = 'dummy-' + post_id + '-' + (index + 1);
    var author = escapeHtml_CUI(comment.author);
    var text = escapeHtml_CUI(comment.comment).replace(/\n/g, '<br>');
    var time = escapeHtml_CUI(comment.time || 'Baru saja');
    return '' +
      '<li id="cui-item-comment-' + id + '" class="depth-1 cui-item-comment">' +
      '<div class="cui-comment-avatar"><img src="' + CUI_DUMMY_AVATAR + '" alt=""></div>' +
      '<div id="cui-comment-' + id + '" class="cui-comment-content">' +
      '<div class="cui-comment-info">' +
      '<a class="cui-commenter-name" href="javascript:void(0)">' + author + '</a>' +
      '<span class="cui-comment-time">' + time + '</span>' +
      '</div>' +
      '<div class="cui-comment-text"><p>' + text + '</p></div>' +
      '</div>' +
      '</li>';
  }

  function ensureCommentBoxScrollable_CUI(post_id) {
    var $box = $('#cui-container-comment-' + post_id).closest('.cui-box');
    $box.css({
      'overflow-y': 'auto',
      'max-height': '40vh',
  ...
  function setCommentCounter_CUI(post_id, count) {
    var $link = $('#cui-link-' + post_id);
    if ($link.find('span').length) {
      $link.find('span').text(String(count));
    }
    $link.attr('title', count + ' Comments');
    var href = $link.attr('href') || '';
    if (/comments=\d+/i.test(href)) {
      $link.attr('href', href.replace(/comments=\d+/i, 'comments=' + count));
    }
  }

  function setAttendanceCounter_CUI(post_id, comments) {
    var hadir = 0;
    var tidakHadir = 0;
    $.each(comments, function (_, item) {
      var val = String(item.konfirmasi || '').toLowerCase();
      if (val === 'hadir') {
        hadir += 1;
    ...
    var $count = $('#invitation-count-' + post_id);
    $count.find('.cui_card-hadir span').first().text(hadir);
    $count.find('.cui_card-tidak_hadir span').first().text(tidakHadir);
  }

  function renderDummyComments_CUI(post_id, submittedData) {
    var comments = buildDummyCommentList_CUI(submittedData);
    var html = '';
    $.each(comments, function (index, item) {
      html += buildDummyCommentItem_CUI(item, post_id, index);
    });
    var $container = $('ul#cui-container-comment-' + post_id);
    $container.html(html).show();
    ensureCommentBoxScrollable_CUI(post_id);
    setCommentCounter_CUI(post_id, comments.length);
    setAttendanceCounter_CUI(post_id, comments);
    ...
    if (num_comments > 0) {
      jQuery.ajax({
        ...
        success: function (data) {
          ...
          var hasCommentMarkup = typeof data === 'string' && data.indexOf('cui-item-comment') !== -1;
          if (!hasCommentMarkup) {
            renderDummyComments_CUI(post_id, null);
            return;
          }
  ...
  function insertComment_CUI(post_id, num_comments) {
    ...
    var submittedData = {
      author: $.trim(comment_form.find('input[name="author"]').val()) || 'Tamu Undangan',
      comment: $.trim(comment_form.find('textarea[name="comment"]').val()),
      konfirmasi: $.trim(comment_form.find('select[name="konfirmasi"]').val())
    };
    ...
    $.ajax({
      ...
      success: function (data, textStatus) {
        ...
        var hasCommentMarkup = typeof data === 'string' && data.indexOf('cui-item-comment') !== -1;
        if (!hasCommentMarkup || data == "error") {
          renderDummyComments_CUI(post_id, submittedData);
      ...
      error: function (XMLHttpRequest, textStatus, errorThrown) {
        renderDummyComments_CUI(post_id, submittedData);
        status.removeClass('cui-loading').html('<p class="cui-ajax-success">' + CUI.thanksComment + '</p>');
      },
      complete: function (jqXHR, textStatus) {
        setTimeout(function () {
